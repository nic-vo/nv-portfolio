...
const Markdown = () => {
	...
	const removeDlUrlFromList = useCallback(
		(qIndex: number) => {
			URL.revokeObjectURL(dlUrls[qIndex].url);
			const filtered = dlUrls.filter((blob) => {
				return dlUrls.indexOf(blob) !== qIndex;
			});
			setDlUrls(filtered);
		},
		[dlUrls],
	...
	const activeUpdateButtonClasser =
		allowLiveUpdates === true ? ` ${style.upActive}` : '';
	const editorClasser = `${style.editor} ${style.pane} ${
		toggleEditor === true
			? style.toggled
			: togglePreview === true
				? style.hidden
	...
	const previewClasser = `${style.preview} ${style.pane} ${
		togglePreview === true
			? style.toggled
			: toggleEditor === true
				? style.hidden
			...
			<div className={style.editDialogue}>
				<div className={editorClasser}>
					<div className={style.header}>
						...
						<p className={style.heading}>editor</p>
						<button
								...
								dlUrls.map((obj, index) => {
									...
									return (
										<li key={`md_dl_${url}`}>
											...
											<button
												onClick={() => {
													setEditorToPreviousInput(rawInput);
												}}
												className={style.button}>
												Edit raw
											</button>
